...
def get_name_from_recipe():
    return get_value_from_recipe(r'name\s*=\s*"(\S*)"', contents).groups()[0]

def get_version_from_recipe():
    return get_value_from_recipe(r'version\s*=\s*"(\S*)"', contents).groups()[0]
